...
export type ArtworkInterpolationTypes =
  | 'nearest'
  | 'box'
  | 'bilinear'
  | 'hamming'
...
export const fetchOrConvertArtwork = async (
  pathOrURL: string,
  dest: string,
  options?: FetchOrConvertOptions
): Promise<Path> => {
  const { textureSize, interpolation, format } = useDefaultOptions<
    FetchOrConvertOptions,
    true
  >({ format: 'png', textureSize: 512, interpolation: 'bilinear' }, options)
  ...
  if (format === 'png_ps3' || format === 'png_wii' || format === 'png_xbox') {
    ...
    const newPath2 = new Path(
      newPath.changeFileName(
        newPath.name.endsWith('_keep') ? newPath.name : `${newPath.name}_keep`
      )
    )
    dest = newPath2.changeFileExt(format)
  } else {
  ...
  if (isStringALink(pathOrURL)) {
    ...
    if (!imageResponse.ok)
      throw new Error(
        `fetchArtworkToPng: URL returned with error with status ${imageResponse.status.toString()}.`
      )
    const imageBuffer = Buffer.from(await imageResponse.arrayBuffer())

    const tempFetchedImagePath = new Path(
      Path.resolve(tempFolder, `${randomBytes(16).toString('hex')}.tmp`)
    )
  ...
  const imageConvScriptPath = new Path(
    Path.resolve(process.cwd(), `src/python/image_converter.py`)
  )
  ...
  if (exec.stdout.startsWith('ImageConverterError'))
    throw new Error(exec.stdout)
